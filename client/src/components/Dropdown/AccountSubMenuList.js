...
const AccountSubMenuList = ({ showAccount, showDropdown }) => {
  ...
  return (
    <div className='sub-menu-list'>
      <button onClick={ showAccount }className='dropdown-account-panel-button'>
      ...
      <div>
        {userMenuList.map(({ title, path }, i) => (
          <div className='dropdown-account-panel-row' key={i}>
            <NavLink onClick={toggleMenu} to={path}>{title}</NavLink>
          </div>
        ))}
      </div>
    </div>
